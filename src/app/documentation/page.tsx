...
// The documentation page reads local Markdown files at request time so newly
// added documentation is visible without rebuilding the app.
...
const documentationGroupLabels: Record<string, string> = {
  ...
  email: "Email",
  migration: "Migration",
  "public-sub-pages": "Public Sub-Pages",
  seo: "SEO",
...
function sortDocuments(a: DocumentationDocument, b: DocumentationDocument) {
...
function groupDocumentationFiles(documents: DocumentationDocument[]): DocumentationGroup[] {
...
function getDocumentSearchText(document: DocumentationDocument) {
  const sectionText = document.sections.flatMap((section) => [
    section.title,
...
function filterDocumentationFiles(documents: DocumentationDocument[], query: string) {
...
function parseMarkdownDocument({
  ...
  let currentSection = {
    ...
    title: "Overview",
    blocks: [] as DocumentationDocument["sections"][number]["blocks"],
  };
...
export default async function DocumentationPage({ searchParams }: DocumentationPageProps) {
  ...
  const rawSearchQuery = Array.isArray(resolvedSearchParams?.q)
    ? resolvedSearchParams?.q[0] || ""
    : resolvedSearchParams?.q || "";
  const searchQuery = rawSearchQuery.trim();
  const documents = readDocumentationFiles();
  const filteredDocuments = filterDocumentationFiles(documents, searchQuery);
  const groups = groupDocumentationFiles(filteredDocuments);
  const totalSections = filteredDocuments.reduce((total, doc) => total + doc.sections.length, 0);
    ...
    <main className="min-h-screen bg-[#f6f7fb] text-slate-950">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 px-4 py-8 sm:px-6 lg:px-10">
        <header className="rounded-2xl border border-slate-200 bg-white px-6 py-6 shadow-sm sm:px-8">
          ...
          <form action="/documentation" className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <label htmlFor="documentation-search" className="text-sm font-bold text-slate-900">
            ...
            <div className="mt-3 flex flex-col gap-3 md:flex-row">
              <input
                ...
                placeholder="Search by topic, file, heading, route, or field"
                className="min-h-12 flex-1 rounded-xl border border-slate-300 bg-white px-4 text-base text-slate-950 outline-none transition focus:border-[#6a5cff] focus:ring-4 focus:ring-[#6a5cff]/15"
              />
              <div className="flex gap-2">
                <button
                  type="submit"
                  className="min-h-12 rounded-xl bg-[#6a5cff] px-5 text-sm font-bold text-white shadow-sm transition hover:bg-[#5a4df0] focus:outline-none focus:ring-4 focus:ring-[#6a5cff]/25"
                >
                  Search
                </button>
                {searchQuery ? (
                  <Link
                    href="/documentation"
                    className="inline-flex min-h-12 items-center rounded-xl border border-slate-300 bg-white px-5 text-sm font-bold text-slate-700 transition hover:border-[#d8d4ff] hover:bg-[#f5f3ff] hover:text-[#5a4df0] focus:outline-none focus:ring-4 focus:ring-[#6a5cff]/15"
                  >
                    Clear
                  </Link>
                ) : null}
            ...
            <p className="mt-3 text-sm text-slate-600">
              {searchQuery
                ? `${filteredDocuments.length} of ${documents.length} documents match "${searchQuery}".`
                : "Search scans document names, folders, headings, lists, and body text."}
            </p>
          </form>
        </header>
        ...
        <div className="grid items-start gap-8 xl:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white px-6 py-12 text-center shadow-sm">
            <p className="text-sm font-semibold uppercase tracking-wide text-[#6a5cff]">No Results</p>
            <h2 className="mt-2 text-2xl font-bold text-slate-950">No documentation found</h2>
            <p className="mx-auto mt-3 max-w-xl text-base leading-7 text-slate-600">
              Try a different keyword, route name, stage number, field name, or folder name.
            </p>
            <Link
              href="/documentation"
              className="mt-6 inline-flex min-h-12 items-center rounded-xl bg-[#6a5cff] px-5 text-sm font-bold text-white shadow-sm transition hover:bg-[#5a4df0] focus:outline-none focus:ring-4 focus:ring-[#6a5cff]/25"
            >
              Clear Search
            </Link>
          </section>
