...
import { getPillarPageContent, getNursingTestBankSubPages, countSubPageQuestions, countPillarPageQuestions } from "@/lib/firestore-operations";
...
export async function generateMetadata(): Promise<Metadata> {
  const result = await getPillarPageContent("nursing-test-bank");
  
  if (result.success && result.data) {
    const data = result.data as any;
    if (data.meta) {
      return {
        title: data.meta.title || "Nursing Test Bank | TeasGurus",
        description: data.meta.description || "",
        keywords: data.meta.keywords || "",
        openGraph: {
          title: data.meta.ogTitle || data.meta.title || "Nursing Test Bank | TeasGurus",
          description: data.meta.ogDescription || data.meta.description || "",
          url: data.meta.canonicalUrl || `${process.env.NEXT_PUBLIC_SITE_URL || "https://teasgurus.com"}/nursing-test-bank`,
          images: [
            {
              url: data.meta.ogImage || "/teas-gurus-logo.png",
              width: 1200,
              height: 630,
              alt: data.meta.title || "Nursing Test Bank",
            },
          ],
        },
        alternates: {
          canonical: data.meta.canonicalUrl || "/nursing-test-bank",
        },
      };
    }
  }
  ...
  return {
    ...
    description: "Comprehensive nursing test bank for exam preparation",
  };
}

export default async function NursingTestBankPage() {
  const result = await getPillarPageContent("nursing-test-bank");
  const subPagesResult = await getNursingTestBankSubPages();
  ...
  const subPagesWithCounts = await Promise.all(
    subPages.slice(0, 2).map(async (subPage: any) => {
      ...
      const questionCount = await countSubPageQuestions("nursing-test-bank", pageSlug);
      return {
        ...subPage,
        questionCount,
      };
  ...
  const pillarPageQuestionCount = await countPillarPageQuestions("nursing-test-bank");
  
  const content: ServiceContent = pageData ? {
    meta: pageData.meta || {
      title: "Nursing Test Bank | TeasGurus",
      description: "Comprehensive nursing test bank for exam preparation",
      keywords: "nursing test bank, nursing exam, exam preparation",
      ogTitle: "Nursing Test Bank | TeasGurus",
      ogDescription: "Comprehensive nursing test bank for exam preparation",
    ...
    hero: pageData.hero || {
      ...
      subtitle: "Comprehensive test bank to help you succeed in your nursing exams.",
      description: "",
    },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-6">
            <h1 className="text-4xl font-bold text-gray-900 mb-3">
              {content.hero.title || "Nursing Test Bank - TEAS Gurus"}
            </h1>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 w-full sm:w-[calc(33.333%-0.67rem)] max-w-sm">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-600 mb-1">
                        {content.hero?.title || "Nursing Test Bank"}
                      </p>
      ...
      <section className="py-20 bg-gradient-to-r from-blue-600 to-purple-600">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-blue-100 mb-8">
            Get expert help with your nursing exam preparation and achieve your
