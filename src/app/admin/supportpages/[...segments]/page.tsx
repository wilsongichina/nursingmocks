...
export default function EditSupportPage({
  ...
  useEffect(() => {
    const resolveParams = async () => {
      ...
      // segments is an array, last element is pageId, rest is serviceId
      ...
      setResolvedParams({ serviceId, pageId });
      setEditablePageId(pageId);
      setEditableServiceId(serviceId);
    };
    resolveParams();
  }, [params]);
...
<p>Mastering ${resolvedParams.pageId.toLowerCase()} is essential for TEAS exam success. With proper preparation and the right resources, you can achieve your goals and excel in your nursing school journey.</p>`,
    ...
    try {
      ...
      if (result.success) {
        ...
        if (
          ...
          setTimeout(() => {
            const newPath = `/admin/supportpages/${targetServiceId}/${targetPageId}`;
  ...
  if (!content || !resolvedParams) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600">{error || "Support page not found"}</p>
        </div>
      </div>
    );
  }
