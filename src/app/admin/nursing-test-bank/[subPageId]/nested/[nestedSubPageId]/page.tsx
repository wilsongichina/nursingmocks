...
function sanitizeNestedPageContent(
  content: NestedPageContent
): NestedPageContent {
  return {
    ...content,
    ...
    cardDescription: replaceLegacyBrandText(content.cardDescription),
...
function buildNestedPageSchema({
  pageName,
  heading,
  description,
  slug,
  ...
  const cleanDescription =
    ...
    `Browse nursing test bank topics and practice questions for ${title}.`;
...
function EditNestedSubPageContent({
  ...
  const loadContent = useCallback(async () => {
    ...
    try {
      ...
      const [parentResult, result] = await Promise.all([
      ...
      if (result.success && result.data) {
        ...
        const loadedPageName =
          pageData.pageName || resolvedParams.nestedSubPageId;
        const loadedHeading =
          pageData.heading ||
          pageData.hero?.title ||
          pageData.pageName ||
        ...
        const generatedSchema = buildNestedPageSchema({
        ...
        const sanitizedContent = sanitizeNestedPageContent(initializedContent);
        ...
        const generatedSchema = buildNestedPageSchema({
          ...
          heading: resolvedParams.nestedSubPageId,
        ...
        const defaultContent: NestedPageContent = {
          ...
          meta: {
            ...
            description: `Content for ${resolvedParams.nestedSubPageId}`,
            keywords: `${resolvedParams.nestedSubPageId}, ${resolvedParams.subPageId}, nursing test bank`,
        ...
        const sanitizedContent = sanitizeNestedPageContent(defaultContent);
        setContent(sanitizedContent);
        setSlug(defaultSlug);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const contentToSave: NestedPageContent = sanitizeNestedPageContent({
  ...
  const handleRegenerateSchema = () => {
    ...
    updateContent(
      "schema",
      buildNestedPageSchema({
        pageName: content.pageName || resolvedParams.nestedSubPageId,
        heading:
          content.heading || content.pageName || resolvedParams.nestedSubPageId,
        description: content.description || content.meta.description,
        slug: slug || content.slug || resolvedParams.nestedSubPageId,
  ...
  const handleCleanLegacyBranding = () => {
    ...
    const cleaned = sanitizeNestedPageContent(content);
    setSlug(replaceLegacyBrandText(slug));
    setContent(cleaned);
    setSuccess("Legacy branding cleaned from the current nested page fields.");
    setTimeout(() => setSuccess(""), 3000);
  };
        ...
        <main className="admin-content min-h-screen bg-gray-50 px-4 py-6 sm:px-6 lg:px-8">
          <div className="w-full max-w-none space-y-6">
            <AdminNotificationRegion
              error={error}
              success={success}
              errorTitle="Unable To Save Nested Sub Page"
              successTitle="Nested Sub Page Saved"
            />
            <AdminPageHeader
              eyebrow="Nursing Test Bank"
              title="Edit Nested Sub Page"
              description="Update the nested page details, SEO, schema, FAQ content, and public body content."
              actions={
            ...
            <section className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(380px,0.85fr)]">
              <AdminFormSection
                ...
                description="Set the nested page label, public URL, visibility, and live-page summaries."
              >
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  ...
                  <AdminFieldGroup
                    label="Display Title"
                    helper="Primary H1 shown on the live nested page."
                  >
                    <input
                      type="text"
                  ...
                  <AdminFieldGroup
                    label="Slug"
                    helper={`Live URL: /${slug || resolvedParams?.nestedSubPageId || "nested-page-slug"}`}
                    required
                  >
                    <AdminSlugField
                  ...
                  <AdminFieldGroup
                    label="Status"
                    helper="Only published nested pages should be presented to students."
                  >
                    <AdminSelectField
                      value={status}
                      onChange={(value) => {
                ...
                <AdminFieldGroup
                  label="Description"
                  helper="Rich text summary for the public nested page."
                >
                ...
                <AdminFieldGroup
                  ...
                  helper="Short supporting copy shown on parent page cards."
                >
                  <textarea
                    value={content.cardDescription || ""}
                    onChange={(event) =>
                      updateContent("cardDescription", event.target.value)
                    }
                    placeholder="Practice nursing concepts, question formats, and explanations for focused test-bank review."
                    rows={3}
                    maxLength={180}
              ...
              >
                ...
                >
                  <input
                    ...
                    placeholder="ATI LPN Exams | NursingMocks"
                    className={adminInputClass}
                  />
                </AdminFieldGroup>
                ...
                >
                  <textarea
                    ...
                    placeholder="Short search result description for this nested page."
                    rows={4}
                    className={adminTextareaClass}
                  />
                </AdminFieldGroup>
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  ...
                  <AdminFieldGroup
                    label="Canonical URL"
                    helper="Use the full canonical URL for this nested page."
                  >
                    <input
                      type="text"
                      ...
                      placeholder="https://www.nursingmocks.com/ati-lpn-exams"
                      className={adminInputClass}
                    />
                  </AdminFieldGroup>
            ...
            <AdminFormSection
              ...
              description="Edit the full public body content and the FAQ block shown on the nested page."
            >
