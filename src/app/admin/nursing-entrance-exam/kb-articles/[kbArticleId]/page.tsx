...
import {
  getNursingEntranceExamKbArticle,
...
function EditKbArticleContent({
  ...
  const loadContent = useCallback(async () => {
    ...
    try {
      ...
      const result = await getNursingEntranceExamKbArticle(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const contentToSave: any = {
        ...
        pillarId: content.pillarId || "nursing-entrance-exam",
      ...
      if (result.success) {
        ...
        if (documentId !== resolvedParams.kbArticleId) {
          ...
          setTimeout(() => {
            router.push(
              `/admin/nursing-entrance-exam/kb-articles/${documentId}`
            );
          }, 1000);
        } else {
