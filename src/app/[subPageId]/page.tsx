...
export const dynamicParams = false; // Only generate pages defined in generateStaticParams
...
// Generate static params for all sub-pages and nested sub-pages
export async function generateStaticParams() {
  try {
    const { getNursingEntranceExamSubPages, getNestedSubPages } = await import(
    ...
    if (subPagesResult.success && subPagesResult.data) {
      for (const subPage of subPagesResult.data) {
        ...
        // Add regular sub-page with -exam suffix
        params.push({ subPageId: `${slug}-exam` });

        // Get nested sub-pages for this sub-page
        const nestedResult = await getNestedSubPages(subPageId);
        if (nestedResult.success && nestedResult.data) {
          for (const nestedSubPage of nestedResult.data) {
            const nestedSlug =
              nestedSubPage.slug ||
              nestedSubPage.id ||
              nestedSubPage.nestedSubPageId;
            // Add nested sub-page URL (parent slug without -exam, then nested slug)
            const parentSlugWithoutExam = slug.endsWith("-exam")
              ? slug.slice(0, -5)
              : slug;
            params.push({
              subPageId: `${parentSlugWithoutExam}-${nestedSlug}-questions`,
            });
          }
        }
      }
    ...
    console.log(`✓ Generated ${params.length} Sub-Page Routes`);
    return params;
  } catch (error) {
...
export async function generateMetadata({
  ...
  // Check if this is a nested sub-page URL pattern
  const nestedSubPageMatch = subPageId.match(/^(.+)-(.+)-questions$/);

  let result: any;
  if (nestedSubPageMatch) {
    // This is a nested sub-page
    // Parent ID in nested URLs doesn't have -exam suffix
    const parentSubPageId = nestedSubPageMatch[1];
    const nestedSubPageId = nestedSubPageMatch[2];
...
export default async function SubPage({
  ...
  // Check if this is a nested sub-page URL pattern: [sub-page]-[nested-sub-page]-questions
  const nestedSubPageMatch = subPageId.match(/^(.+)-(.+)-questions$/);
  ...
  if (nestedSubPageMatch) {
    // This is a nested sub-page
    // Parent ID in nested URLs doesn't have -exam suffix
    parentSubPageId = nestedSubPageMatch[1];
    nestedSubPageId = nestedSubPageMatch[2];
  ...
  const content: ServiceContent = {
    ...
    meta: pageData.meta || {
      title: `${subPageId} | TeasGurus`,
      description: `Content for ${subPageId}`,
      keywords: `${subPageId}, nursing entrance exam`,
            ...
            <div className="mt-6">
              <div className="flex flex-wrap justify-center gap-4">
                ...
                {nestedSubPages.map((nestedSubPage: any, index: number) => {
                  ...
                  // Card configuration - cycle through colors
                  const cardConfigs = [
                    {
                    ...
                    },
                    {
                      ...
                      icon: <MedalIcon className="w-6 h-6 text-white" />,
                    },
                  ];
                  ...
                  // Get the slug for URL generation (without -exam for nested sub-pages)
                  // Use lookupId (which already has -exam stripped) as the base, or fallback to pageData.slug/subPageId
                  // Always ensure -exam is removed for nested sub-page URLs
                  let baseSlug = lookupId; // lookupId already has -exam stripped
                  if (!baseSlug) {
                    baseSlug = pageData?.slug || subPageId;
                  }
                  // Remove -exam suffix if present (safety check)
                  const parentUrlSlug = baseSlug.endsWith("-exam")
                    ? baseSlug.slice(0, -5)
                    : baseSlug;
                  ...
                  return (
                    <Link
                      key={nestedPageId}
                      href={`/${parentUrlSlug}-${nestedPageId}-questions`}
