...
import type { KnowledgeHubSubPage } from "./loadKnowledgeBaseHubSubPages";
          ...
          {subPages.length === 0 ? (
            ...
            subPages.map((subPage) => {
              ...
              const nestedNames = nestedSubPages
                ...
                .map(
                  (nested) =>
                    nested.pageName || nested.title || nested.heading || nested.id
                )
                .filter(Boolean);
              ...
              return (
                <article key={subPage.id} className="hub-card relative">
                  <Link
                    href={pageUrl}
                    className="absolute inset-0 z-[1] rounded-[20px]"
                    aria-label={`${String(pageName)} Knowledge Base`}
                    prefetch={false}
                  />
                  <div className="hub-cloud-main"></div>
                  <div className="hub-cloud-small"></div>

                  <div className="hub-chip-row relative z-[2]">
                    <div className="hub-tag">{pillarLabel}</div>
                    <div className="hub-mini-pill">{miniPillText}</div>
                  </div>

                  <h2 className="hub-title relative z-[2]">{pageName} Knowledge Base</h2>
                  <p className="hub-desc relative z-[2]">
                    {getDescription()}
                  </p>
                  <p className="hub-links relative z-[2]">
                      ...
                      subPage.kbArticles.map((kbArticle, index) => {
                        ...
                        return (
                          <span key={kbArticle.id}>
                            {index > 0 && " • "}
                            <a href={kbUrl} className="relative z-[2]">
                              {kbName}
                            </a>
                          </span>
                        );
                  ...
                  <div className="hub-footer relative z-[2]">
